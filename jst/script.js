...
/**************************************/
window.max = data.length - 1; // Определение количества фраз

/******Добавление новой каррточки***********/
...
var count=0; // счетчик количества карточек в колонке
/*****Реализация добавления карточек для 1-й колонки******/
function newCardOne(randomText){
	...
	if(count<5){					
		...
		/****Реализация смены текста на текс с переводом при клике****/
		...
		div.ondblclick = function(){
			...
			if(count+1==5) document.querySelector(".add").style.display = "none"; //Ограничение количества карточек и скрытие панели допавления карточки
			count++;
		}
	}
/*****Реализация добавления карточек для 2-й колонки. Аналогично 1-й******/
	var countTwo = 0;
...
/*****Реализация добавления карточек для 3-й колонки. Аналогично 1-й******/
	var countThree = 0;
	...
	var perem; 
	var ren = [];
	function randText(){								//Сортировка по убыванию кол-ва слов, случайно взятого текста
...
/*Создание 3-х карточек 1-й колонки*/
	function colOne(){
		randText();
		for(var j = 0; j<3; j ++){
			newCardOne(ren[j]);
	...
	colOne();
	/*Создание 5ти карточек 2-й колонки*/
	function colTwo(){
		randText();
		for(var j = 0; j<5; j ++){
			newCardTwo(ren[j]);
	...
	colTwo();
	/*Создание 5ти карточек 3-й колонки*/
	function colThree(){
		randText();
		for(var j = 0; j<5; j ++){
			newCardThree(ren[j]);
